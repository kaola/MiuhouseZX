...
public class Base64 {
	...
	/*
	 * Declare working variables including an array of bytes that will contain the decoded data to be returned to the caller. Note that the decoded array is about 3/4 smaller than the input. This
	 * is because every group of 4 bytes is being encoded into 3 bytes.
	 */
	int iSrcIdx; // index into source (byteData)
	...
	/*
	 * remove any '=' chars from the end of the byteData they would have been padding to make it up to groups of 4 bytes note that I don't need to remove it just make sure that when progressing
	 * throug array we don't go past reviSrcIdx ;-)
	 */
	...
	/*
	 * Set byteDest, this is smaller than byteData due to 4 -> 3 byte munge. Note that this is an integer division! This fact is used in the logic l8r. to make sure we don't fall out of the array
	 * and create an OutOfBoundsException and also in handling the remainder
	 */
	byte byteDest[] = new byte[((reviSrcIdx * 3) / 4)];

	/*
	 * Convert from Base64 alphabet to encoded data (The Base64 alphabet is completely documented in RFC 1521.) The order of the testing is important as I use the '<' operator which looks at the
	 * hex value of these ASCII chars. So convert from the smallest up
	 * 
	...
	/*
	 * 4bytes -> 3bytes munge Walk through the input array, 32 bits at a time, converting them from 4 groups of 6 to 3 groups of 8 removing the two unset most significant bits of each sorce byte
	 * as this was filler, as per Base64 spec. stop before potential buffer overun on byteDest, remember that byteDest is 3/4 (integer division) the size of input and won't necessary divide
	 * exactly (ie iDestIdx must be < (integer div byteDest.length / 3)*3 see http://www.javaworld.com/javaworld/javatips/jw-javatip36-p2.html for example
	 */
